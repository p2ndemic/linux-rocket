Funny you suggest that cause I tried it earlier. It indeed stopped flickering but streaming only worked when the overlay was open. Otherwise it would just freeze.

I spent all night today trying to get this fixed and I decided to just look at my steam deck and see what that does. I essentially coppied the script from the steam deck that creates the wayland session.
```bash
#!/bin/bash
...
ui_background=/usr/share/plymouth/themes/steamos/steamos.png
...
gamescope \
    ...
    -W 3840 -H 2160 -w 3840 -h 2160 -r 120 --adaptive-sync \
	-e -R "$socket" -T "$stats"
```

And then I created a systemd service for steam.
```
[Unit]
Description=Steam Launcher

[Service]
ExecStart=/home/kamil/.local/bin/steam-launcher.sh
ExecStop=/bin/bash -c 'kill -TERM $(pgrep -P $MAINPID || echo $MAINPID)'
Type=exec
EnvironmentFile=/run/user/1000/gamescope-environment 
```

That launches a file that I also coppied and modified from the steamdeck
```bash
#!/bin/bash
...
And that worked. All my issues seem to be solved but it's such an ugly solution.
